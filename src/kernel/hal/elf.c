...
//     size_t envc = 0;
//     if (envp) {
//         while (envp[envc] && envc < MAX_EXEC_ARGS) envc++;
//     }
...
//         size_t padded = (len + 7) & ~7ULL; // 8-byte align

//         sp -= padded;
//         env_addrs[i] = sp;

//         // copy string into stack memory
//         for (size_t off = 0; off < len; off += 8) {
//             uint64_t word = 0;
//             for (int b = 0; b < 8 && off + b < len; b++) {
//                 ((uint8_t*)&word)[b] = (uint8_t)s[off + b];
//             }
...
//     // Align stack to 16 bytes for ABI before pushing pointers
//     sp &= ~0xFULL;

//     // 3) Push auxiliary vector (auxv) entries
//     uint64_t auxv[32];
//     int ax = 0;
...
//     // 4) Push envp NULL terminator
//     sp -= 8;
//     u64_store(p, sp, 0);

//     // 5) Push envp pointers (0..envc-1)
//     for (size_t i = 0; i < envc; i++) {
//         sp -= 8;
//         u64_store(p, sp, env_addrs[i]);
//     }

//     // 6) Push argv NULL terminator
//     sp -= 8;
//     u64_store(p, sp, 0);

//     // 7) Push argv pointers (0..argc-1)
//     for (size_t i = 0; i < argc; i++) {
//         sp -= 8;
//         u64_store(p, sp, arg_addrs[i]);
//     }

//     // 8) Ensure 16-byte alignment for entry (RSP % 16 == 8)
//     if ((sp & 0xF) != 8) {
//         sp -= 8;
...
// /* ---------- exec from fd ---------- */
// // exec_elf_from_fd.c
// // Modified exec_elf_from_fd: allocate a kernel stack for the new process and install TSS
// // Assumes the following helpers exist in your kernel:
// //   - kmalloc / kfree (used earlier in this file)
// //   - pmm_alloc_page() -> returns physical address of a free page (uint64_t) or 0 on failure
// //   - phys_to_virt(pa) -> kernel virtual address for a physical page
// //   - pmm_free_page(pa) -> free a physical page (optional, used on error cleanup)
// //   - PAGE_SIZE constant
// //   - x64_TSS_Install(uintptr_t stack_top) as provided earlier
// // Adjust allocation strategy if your kernel provides a different page allocator.
...
//     //
//     // === NEW: allocate a kernel stack for this process and install it in the TSS ===
//     //
//     // We allocate a small kernel stack (KERNEL_STACK_PAGES pages) in kernel memory,
//     // record base/top in the process struct and call x64_TSS_Install(top).
//     //
// #define KERNEL_STACK_PAGES 4
// #define KERNEL_STACK_SIZE  (KERNEL_STACK_PAGES * PAGE_SIZE)

//     {
//         uint64_t allocated_phys[KERNEL_STACK_PAGES];
//         int allocated = 0;
...
//             uint64_t pa = pmm_alloc_page();
//             if (!pa) {
//                 log_critical("EXEC", "exec_elf_from_fd: pmm_alloc_page failed for kernel stack (i=%d)", i);
//                 // cleanup any pages already allocated
//                 for (int j = 0; j < allocated; ++j) {
//                     #ifdef HAVE_PMM_FREE_PAGE
//                     pmm_free_page(allocated_phys[j]);
//                     #endif
//                 }
//                 kfree(ph);
//                 // Optionally free process 'p' and its page directory here
//                 return -1;
//             }
//             allocated_phys[allocated++] = pa;
//             void *kva = (void *)phys_to_virt(pa);
//             memset(kva, 0, PAGE_SIZE);
//             if (i == 0) {
//                 kstack_base = (uintptr_t)kva;
//             }
//         }

//         // If pmm_alloc_page returns contiguous physical pages and phys_to_virt is a direct mapping,
//         // the virtual addresses will be contiguous. We compute top accordingly.
//         // If your allocator does not guarantee contiguous pages, replace this with a proper
//         // contiguous allocation routine or use kmalloc backed by pages.
//         p->kernel_stack_base = kstack_base;
//         p->kernel_stack_top  = kstack_base + KERNEL_STACK_SIZE;

//         // Ensure 16-byte alignment of top
//         p->kernel_stack_top &= ~0xFULL;

//         log_info("EXEC", "Allocated kernel stack for pid=%d base=%p top=%p",
//                  p->pid, (void*)p->kernel_stack_base, (void*)p->kernel_stack_top);

//         // Install TSS / set g_syscall_rsp0 to the kernel stack top
//         x64_TSS_Install(p->kernel_stack_top);
//     }
//     //
//     // === END kernel stack allocation ===
//     //
